...
$models/revell_{\mathrm{phreeqc}}_{\mathrm{screening}}_{\mathrm{template}}.phr$ 给出了浅部 Ca-HCO3 端元和深部 Ca-Na-Cl 端元的占位输入结构。执行前必须填入样品级温度、pH、pe/Eh、Ca、Na、K、Mg、Cl、SO4、碱度、Fe、U 和气相 H2。优先输出应包括离子强度、饱和指数、H2、U-碳酸盐物种、HS- 与 SO4 物种。没有这些样品级输入，不应报告“Revell PHREEQC 结果”。
...
| $G_{H_2}$ | H2 辐解 G 值 | $\mathrm{mol}\,\mathrm{J}$-1 |
| $\epsilon_w$ | 水可达能量分数 | dimensionless |
| $R_{H_2}$ | H2 生成率 | $\mathrm{mol}\,\mathrm{m}$-3 yr-1 |
...
- $data/lithology_{\mathrm{radioelements}}.csv$
- $data/revell_{\mathrm{public}}_{\mathrm{parameters}}.csv$
- $data/derived_{\mathrm{radiolysis}}_{\mathrm{screening}}.csv$
- $data/groundwater_{\mathrm{depth}}_{\mathrm{framework}}.csv$
- $data/scenario_{\mathrm{matrix}}.csv$
- $data/equation_{\mathrm{registry}}.csv$
- $figures/fig01_{\mathrm{radioelements}}_{\mathrm{heat}}_{\mathrm{h2}}.svg$
- $figures/fig02_{\mathrm{energy}}_{\mathrm{partition}}_{\mathrm{attenuation}}.svg$
- $figures/fig03_{\mathrm{groundwater}}_{\mathrm{depth}}_{\mathrm{framework}}.svg$
- $figures/fig04_{\mathrm{h2}}_{\mathrm{closed}}_{\mathrm{system}}_{\mathrm{pressure}}.svg$
- $figures/fig05_{\mathrm{thmc}}_{\mathrm{reaction}}_{\mathrm{network}}.svg$
- $models/revell_{\mathrm{phreeqc}}_{\mathrm{screening}}_{\mathrm{template}}.phr$
