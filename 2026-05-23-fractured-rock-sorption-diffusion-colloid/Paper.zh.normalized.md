...
- $data/skb_{\mathrm{forsmark}}_{\mathrm{kd}}_{\mathrm{selected}}.csv$：从 SKB R-10-48 Table 6-1 整理的目标核素 Kd。
- $data/skb_{\mathrm{transport}}_{\mathrm{reference}}_{\mathrm{parameters}}.csv$：从 SKB TR-10-50 整理的 $\theta_m$ 与 $D_e$。
- $data/derived_{\mathrm{retardation}}_{\mathrm{and}}_{\mathrm{diffusion}}.csv$：由 Kd、孔隙度和扩散系数推导的迟滞因子和扩散深度。
- $data/nagra_{\mathrm{cfm}}_{\mathrm{desorption}}_{\mathrm{rates}}.csv$ 与 $data/nagra_{\mathrm{cfm}}_{\mathrm{filtration}}_{\mathrm{parameters}}.csv$：Grimsel CFM 胶体解吸、过滤和可逆附着参数。
...
PHREEQC 适合处理水化学形态、饱和指数、离子交换和表面络合原型。本文提供 $models/phreeqc_{\mathrm{sorption}}_{\mathrm{speciation}}_{\mathrm{template}}.phr$。建议使用流程为：
...
- $data/source_{\mathrm{bibliography}}.csv$：公开证据源列表。
- $data/skb_{\mathrm{forsmark}}_{\mathrm{kd}}_{\mathrm{selected}}.csv$：目标核素 Kd 表。
- $data/skb_{\mathrm{transport}}_{\mathrm{reference}}_{\mathrm{parameters}}.csv$：孔隙度与有效扩散系数。
- $data/derived_{\mathrm{retardation}}_{\mathrm{and}}_{\mathrm{diffusion}}.csv$：迟滞因子和扩散深度演算。
- $data/nagra_{\mathrm{cfm}}_{\mathrm{field}}_{\mathrm{summary}}.csv$：Grimsel CFM 现场试验汇总。
- $data/nagra_{\mathrm{cfm}}_{\mathrm{desorption}}_{\mathrm{rates}}.csv$：胶体结合核素解吸速率。
- $data/model_{\mathrm{alignment}}_{\mathrm{matrix}}.csv$：命题—数据—模型对齐矩阵。
- `figures/*.svg`：论文图件。
