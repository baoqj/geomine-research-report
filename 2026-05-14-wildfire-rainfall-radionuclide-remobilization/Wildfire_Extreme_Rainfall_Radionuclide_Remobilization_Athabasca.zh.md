...
ECCC Key Lake 2020-2024 年日降雨合并后有 1827 个日记录。2020 年年降水量为 618.0 mm，最大日降雨 47.1 mm；2023 年为 342.2 mm，最大日降雨 43.5 mm；2024 年为 388.1 mm，最大日降雨 20.4 mm。WSC 06DA004 同期有 1827 个日流量记录，2020 年平均流量 76.298 m3 s^-1，2024 年平均流量 26.000 m3 s^-1。
...
其中，$Q(t)$ 为流量，单位 m3 s^-1；$C_{k,d}$ 为溶解态浓度，单位 Bq m^-3 或 kg m^-3；$TSS$ 为悬浮颗粒质量浓度，单位 kg m^-3；$C_{k,p}$ 为颗粒相浓度，单位 Bq kg^-1 或 kg kg^-1。该式说明，仅凭日均水样浓度可能低估第一冲刷脉冲，因为 $TSS(t)$ 和 $Q(t)$ 的峰值常与短历时强降雨同步。
...
其中，$\rho_b$ 为沉积物或土壤干容重，kg m^-3；$\theta$ 为含水率或孔隙度；$D_L$ 为纵向弥散系数，m2 s^-1；$v$ 为孔隙水平均流速，m s^-1；$r_k$ 为反应项，包含络合、沉淀、吸附、氧化还原和胶体过滤等过程。该方程表明，野火影响并不只表现为源项变化，还可能通过改变 $\theta$、$K_d$、DOC、pH、Eh 和颗粒物浓度改变迁移速度。
...
| $S_k^{ash}$ | 灰烬/火烧土壤释放源项 | Bq s^-1 或 kg s^-1 |
| $S_k^{tail}$ | 尾矿/废石/设施再暴露源项 | Bq s^-1 或 kg s^-1 |
| $Q(t)$ | 流量 | m3 s^-1 |
| $C_{k,d}$ | 溶解态核素浓度 | Bq m^-3 或 kg m^-3 |
| $C_{k,p}$ | 颗粒相核素浓度 | Bq kg^-1 或 kg kg^-1 |
| $TSS$ | 悬浮颗粒物浓度 | kg m^-3 |
| $K_{d,k}$ | 分配系数 | m3 kg^-1 |
| $R_k$ | 阻滞因子 | 无量纲 |
| $\rho_b$ | 土壤或沉积物干容重 | kg m^-3 |
| $\theta$ | 孔隙度或含水率 | 无量纲 |
| $D_L$ | 纵向弥散系数 | m2 s^-1 |
| $v$ | 平均孔隙水流速 | m s^-1 |
| $\lambda_k$ | 放射性衰变常数 | s^-1 |
