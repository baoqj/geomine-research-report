...
```math
S_{H2}^{rad} \approx 1.6 \ \mathrm{nmol \ m^{-3}_{rock} \ yr^{-1}}
```
...
C_{H2,pore}^{1 Myr}
\approx
\frac{1.6 \times 10^{-9} \ \mathrm{mol \ m^{-3}_{rock} \ yr^{-1}} \times 10^6 \ \mathrm{yr}}
...
\frac{nRT}{V_p}
=
\frac{1.6 \times 10^{-3} \ \mathrm{mol} \times 8.314 \ \mathrm{J \ mol^{-1} K^{-1}} \times 293 \ \mathrm{K}}
{0.0045 \ \mathrm{m^3}}
...
```math
S_i^{rad} = G_i \, \dot{E}_{abs}
```

| 符号 | 含义 | 单位 |
|---|---|---|
| S_i^rad | 物种 i 的辐解生成率 | mol m^-3 yr^-1 |
| G_i | 物种 i 的辐解产额，若以 molecules/100 eV 给出需换算为 mol/J | mol J^-1 |
| E_abs dot | 水或矿物-水界面吸收的辐射能量率 | J m^-3 yr^-1 |
...
|---|---|---|
| rho_r | 岩石密度 | kg m^-3 |
| C_j | U、Th、K 的质量分数或活度换算浓度 | kg kg^-1 |
| A_j | 单位质量元素或核素的活度 | s^-1 kg^-1 |
...
```math
\frac{\partial(\phi S_w C_{H2})}{\partial t}
=
\nabla \cdot (\phi S_w D_e \nabla C_{H2})
- \nabla \cdot (\mathbf{q} C_{H2})
+ S_{H2}^{rad}
+ S_{H2}^{corr}
- R_{bio}
- R_{min}
...
| C_H2 | 溶解 H2 浓度 | mol m^-3 water |
| D_e | 有效扩散系数 | m2 s^-1 |
| q | Darcy flux | m s^-1 |
| S_H2^rad | host-rock 辐解 H2 源项 | mol m^-3 rock s^-1 |
| S_H2^corr | 容器/钢材腐蚀等工程源项 | mol m^-3 s^-1 |
| R_bio | 微生物耗氢 | mol m^-3 s^-1 |
| R_min | 矿物/氧化剂耗氢 | mol m^-3 s^-1 |
| R_gas | 气液相转移项 | mol m^-3 s^-1 |
...
R_{SRB} =
k_{SRB} X
\frac{C_{H2}}{K_{H2}+C_{H2}}
\frac{C_{SO4}}{K_{SO4}+C_{SO4}}
f(T,pH,a_w)
```

```math
J_{HS^-}^{Cu} = -D_{HS^-}^{bentonite} \nabla C_{HS^-}
```
...
| 主岩 Th | C_Th | IG_BH04 平均 1.24 ppm | 同上 |
...
| 深部水龄 | tau_w | noble gas 指示 >1 Ma | 4He、36Cl、81Kr、U-series 联合约束 |
| H2 辐解源项 | S_H2^rad | Higgins et al. 约 1.6 nmol m^-3 rock yr^-1 | 不确定性分布、岩性分层和 dose partition |
| 硫化物浓度 | C_HS | 地下水总溶解硫化物 <0.02 mg/L 检出限 | 长期监测、低检出限、微生物培养 |
...
| sulfate reduction rate | R_SRB | 未公开定量 | tracer incubation、metagenomics、bentonite tests |
| H2 solubility | K_H | 未公开 | Revell salinity, pressure, temperature correction |
| Cu corrosion flux | J_corr | 未公开 | sulfide diffusion + copper surface kinetics |
...
3. IG_BH04 的 K、U、Th 平均值分别约为 0.28%、0.53 ppm、1.24 ppm，支持低天然辐解源项。Higgins et al. 2025 直接针对 Revell 的 H2 产率约为 1.6 nmol m^-3 rock yr^-1。
